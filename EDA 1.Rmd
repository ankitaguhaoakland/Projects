...
pollutants1 <- count(eda1, 'parameter_name')  %>% filter(freq > 3000)
...
```
Looks like some of the top Pollutants in the entire datasets are Benzene, Carbon monoxide, Lead, NO2, NO etc along with Ambient Max and Min Temperature, Wind Speed etc captured in that particular day when the pollutant data was calculated.
...
# Visualization
ggplot(data = pollutants1987) + geom_histogram(aes(x = freq, binwidth = 300000, fill = parameter_name)) + scale_x_log10()
```
...
# Visualization
ggplot(data = pollutants2017) + geom_histogram(aes(x = freq, binwidth = 300000, fill = parameter_name)) + scale_x_log10()
```
...
# Visualization
ggplot(data = pollutants2) + geom_histogram(aes(x = freq, binwidth = 300000, fill = parameter_name)) + scale_x_log10()
```

Not a great visual, but we have an idea which Pollutants are kind of topping the datasets. 

Some of the top contributing Pollutants that are apparently present in the entire datasets for both the year of 1987 & 2017 are Arsenic,Cadmium, Carbon Monoxide, along with some other Pollutants such as Suspended Particulate (TSP), Sulfur Dioxide etc.  
...
**Conclusion**
+ Due to the huge nature of the datasets, some of the Map might get stuck when the entire RMD File is run as a whole. However if that code chunk is run independently after a little delay, it runs successfully showing the Map Data of the Pollutants so collected.

+ The zoom function so used in the code snippets for the Maps, can be increased or decreased (by changing the number) to focus into the areas from where the data has been collected. 

+ We tried to keep the zoom to a number so as to have an entire arial view of the observation Pollutants data so collected in the US map. 
