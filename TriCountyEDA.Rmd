...
map_MI <- get_map(location = c(lon = mean(mi_ozone$longitude), lat = mean(mi_ozone$latitude)), zoom = 6,
                      maptype = "satellite", scale = 2)

ggmap(map_MI) +
geom_point(data = mi_ozone, aes(x = longitude, y = latitude, colour = county_name, alpha = 0.7), size = 5, shape = 21) + guides(fill=FALSE, alpha=FALSE, size=TRUE)
...
ThreeCountyMap <- qmap("michigan", zoom = 7, color = "color", legend = "topleft")
ThreeCountyMap +
geom_point(aes(x = longitude, y = latitude, colour = county_name, size = county_name),
